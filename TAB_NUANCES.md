...
- **Include current date (T) always**: The 20-day table must always include the current trading day (e.g. Feb 18 when run on Feb 18). Define T as the latest trading day up to and including **today** when today is a weekday: `last_in_data = benchmark_data.index[-1].date()`, `today = pd.Timestamp.now().normalize().date()`, `is_today_business = (dayofweek < 5)`, `t_date = max(last_in_data, today) if is_today_business else last_in_data`. Then `dates_20 = pd.bdate_range(end=t_date, periods=20, freq='B')`. If the last element of `dates_20` is not `t_date`, replace it so the "(Current day)" row is never missed.
